        ...
        batchSettingsContainer.find('div.batch-settings-info').qtip($.extend({},
            nf.Common.config.tooltipConfig,
            {
                content: (portType === 'input'
                    ? 'The batch settings to control how this UNLSH sends data to the remote input port in a transaction.'
                        + ' This UNLSH will transfer as much flow files as they are queued in incoming relationships,'
                        + ' until any of these limits is met.'
                        + ' If none of these setting is specified, this UNLSH uses 500 milliseconds batch duration by default.'
                    : 'The batch settings to tell the remote UNLSH how this UNLSH prefers to receive data from the remote output port in a transaction.'
                        + ' The remote UNLSH will use these settings as a hint to control batch data transferring.'
                        + ' However, actual behavior depends on the version of remote UNLSH instance.'
                        + ' Recent version of UNLSH uses 5 seconds for batch duration if none of these setting is specified.')
            }));
